        ...
        <div className="container" style={{display:"flex", justifyContent:"center", marginBottom: 15}}>
            <div className="row justify-content-center border border-dark rounded p-3">
                <div className="row p-2">
                    ...
                    <button onClick={() => setCount(count + 1)} className={count>=stock ? "col-2 btn btn-outline-secondary disabled":"col-2 btn btn-outline-dark"}>+</button>
                </div>
                <div>
                    {/*Botón que resta al contador, se desactiva si no hay stock o si la cantidad actual es menor o igual a 0*/}
                    <button onClick={() => onAdd(count)} className={(stock<=0 || count<=0) ? "col-12 btn btn-outline-secondary disabled":"col-12 btn btn-outline-dark"}>Agregar al carrito</button>
                </div>
            </div>
        </div>
    );
